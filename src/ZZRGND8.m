ZZRGND8 ;ISC/GRK - STRUCTURED INDEX ;01/04/2000  14:29
...
EE I LIN="" Q
 ...
 D SEP S EOC=0,COM=$$CASE^ZZRGND52($P(ARG,":")),CM=$P($G(IND("CMD",COM)),"^") I CM="" G ERR
...
XCR ;Option entry point
 K ^UTILITY($J) D ASKRTN^ZZRGND6 G EXIT:NRO<1 S %ZIS="M" D ^%ZIS Q:POP  U IO(0)
 I $D(IO("Q")) S ZTRTN="XC2^ZZRGND8",ZTSAVE("^UTILITY($J,")="",ZTDESC="Structured print" D ^%ZTLOAD G EXIT
XC2 U IO I '$D(INDXDT) D NOW^%DTC S INDXDT=$E(%,2,3)_"/"_$E(%,4,5)_"/"_$E(%,6,7)
 D BUILD^ZZRGND7
 S RTN="" F  S RTN=$O(^UTILITY($J,RTN)) Q:RTN=""  D  D ZZRGND8
 . D LOAD^ZZRGNDX
