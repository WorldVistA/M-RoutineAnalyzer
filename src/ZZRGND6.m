ZZRGND6 ;ISC/REL,GRK - GET SET OF ROUTINES TO INDEX ;07/22/08  13:54
 ...
 N INDHDR,INP,LI,LL,LN,LV,N,NOA,OP,PG,QUES,RN,T,XCNP,XX1,XX2,Z,Z1,INDXDT,FILE
 K ^UTILITY($J),ZTSK,ZTDTH,ZTIO
 S:'$D(DTIME)#2 DTIME=360
 D HOME^%ZIS,HDR^ZZRGND7
 D ASKRTN,PARAM
 I $D(^DIC(9.4))!$D(^DIC(9.6)) D ^ZZRGND10 G END:$D(DUOUT) S INDDA=DA I DA>0 D ANS("Include the compiled template routines: N//","NY") G:X="^" END S:"Nn"'[X INP(9)=1
 G END:(NRO'>0)&(INDDA'>0)
...
DEVICE W:NRO>2 !!,$C(7),"This report could take some time, Remember to QUEUE the report.",! K IOP,%ZIS S %ZIS="QM",%ZIS("B")="" D ^%ZIS K %ZIS I POP W !,$C(7),"XINDEX terminated.  No device specified." G END
 ...
 ;
END K ZTSK,%ZIS G CLEAN^ZZRGND5
 ;
...
QUICK(RL) ;Quick Report, Just errors on some routines.
 ...
 K ^UTILITY($J),ZTSK,ZTDTH,ZTIO
 D HOME^%ZIS I '$D(IOP) D HDR^ZZRGND7
...
NY S %="N" D RD Q:"^YyNn"[X  W:$D(HELP) !,HELP W !,"Please enter 'N' or return for NO, 'Y' for YES" S Y=0 Q
 ;
RD R X:DTIME S:X["^" X="^" S X=$E($$CASE^ZZRGND52(X)_%) Q
 ;
