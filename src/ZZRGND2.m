ZZRGND2 ;
 ;;7.3;TOOLKIT;
% S LINE=CHGRB,(CM,COM)="" F I=0:0 S STR=$P(LINE,$C(9),1),LINE=$P(LINE,$C(9),2,999),NOA=0 D:STR]"" ARGG Q:LINE']""
 Q
 ;
...
GLO(LV,LI,S,S1) ;
 ...
 . S S=S_S1 
 . S Y=$$PEEKDN(.LV,.LI) 
 . S:(Y?1N.NP)!($A(Y)=34)!("^$J^$I^$H^"[(U_Y)) S=S_Y
 D ST(LOC,S) 
 I S1="(" D
 . D INC2(.LV,.LI,.S,.S1) 
 . D DN(.LV,.LI,S)
